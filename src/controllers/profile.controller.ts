...
export const savePaydayConfig = async (req: Request, res: Response) => {
  const {
    type, fixedDay, businessDaysBefore, customText,
    accountId, amount, currency, label,
  } = req.body;
  ...
  if (!type || !VALID_TYPES.includes(type)) {
    return res.status(400).json({ success: false, error: 'Tipo de día de pago inválido' });
  }
  if (type === 'fixed_day' && (!fixedDay || fixedDay < 1 || fixedDay > 31)) {
    return res.status(400).json({ success: false, error: 'Día fijo debe ser entre 1 y 31' });
  }
  if (type === 'business_days_before_end' && (!businessDaysBefore || businessDaysBefore < 1)) {
    return res.status(400).json({ success: false, error: 'Días hábiles antes del fin deben ser al menos 1' });
  }
